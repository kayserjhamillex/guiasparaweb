...
jQuery(document).ready(function($) {
  function Calendar(element, Args) {
    ...
    var eventColors = [
      // https://www.materialui.co/colors (800)
      '#C62828', // Red
      '#AD1457', // Pink
      '#6A1B9A', // Purple
      '#4527A0', // Deep Purple
    ...
    this.conf = {
      locale: Args.locale ? 'en' : Args.locale,
      view: 'week',
      ...
      weekday: {
        ...
        dayline: {
          ...
          format: Args.weekday ? (Args.weekday.dayline ? (Args.weekday.dayline.format ? Args.weekday.dayline.format : 'dddd MM/DD') : 'dddd MM/DD') : 'dddd MM/DD',
      ...
      month: {
        ...
        day: {
          format: Args.month ? (Args.month.day ? (Args.month.day.format ? Args.month.day.format : 'MM/DD') : 'MM/DD') : 'MM/DD'
        }
      },
  ...
  Calendar.prototype.init = function() {
    ...
    $(this.element).trigger('Calendar.init', [this, this.getPrevViewInterval(), this.getViewInterval(), this.getNextViewInterval()]);
    $(this.element).html('');
  ...
  Calendar.prototype.defaultEvents = function() {
    if (this.binded == undefined) {
      ...
      $(self.element)
        .off('Calendar.event-mouseenter', eventMouseenterDefault)
        .on('Calendar.event-mouseenter', eventMouseenterDefault);
      $(self.element)
        .off('Calendar.daynote-mouseenter', eventMouseenterDefault)
        .on('Calendar.daynote-mouseenter', eventMouseenterDefault);
      var eventMouseleaveDefault = function(event, self, elem) {
        if (!event.isDefaultPrevented()) {
          elem.css('z-index', 'auto').animate(
            {
      ...
      $(self.element)
        .off('Calendar.event-mouseleave', eventMouseleaveDefault)
        .on('Calendar.event-mouseleave', eventMouseleaveDefault);
      $(self.element)
        .off('Calendar.daynote-mouseleave', eventMouseleaveDefault)
        .on('Calendar.daynote-mouseleave', eventMouseleaveDefault);
        ...
        if (!event.isDefaultPrevented()) {
          ...
          modal.find('.modal-body').append(
            $('<h4>')
              .append(
                $(this)
                  .closest('.calendar-events-day')
                  .find('span')
                  .html()
              )
              .append(' ')
              .append($('<small>').text(elem.find('.event-date').text()))
          );
      ...
      $(self.element)
        .off('Calendar.event-click', eventClickDefault)
        .on('Calendar.event-click', eventClickDefault);
      $(self.element)
        .off('Calendar.daynote-click', eventClickDefault)
        .on('Calendar.daynote-click', eventClickDefault);
      ...
      $(self.element)
        .off('Calendar.category-event-click', eventCategoryClickDefault)
        .on('Calendar.category-event-click', eventCategoryClickDefault);
      $(self.element)
        .off('Calendar.category-daynote-click', eventCategoryClickDefault)
        .on('Calendar.category-daynote-click', eventCategoryClickDefault);
      ...
      $(self.element)
        .off('Calendar.category-event-mouseenter', eventCategoryMouseenterDefault)
        .on('Calendar.category-event-mouseenter', eventCategoryMouseenterDefault);
      $(self.element)
        .off('Calendar.category-daynote-mouseenter', eventCategoryMouseenterDefault)
        .on('Calendar.category-daynote-mouseenter', eventCategoryMouseenterDefault);
      ...
      $(self.element)
        .off('Calendar.category-event-mouseleave', eventCategoryMouseleaveDefault)
        .on('Calendar.category-event-mouseleave', eventCategoryMouseleaveDefault);
      $(self.element)
        .off('Calendar.category-daynote-mouseleave', eventCategoryMouseleaveDefault)
  ...
  Calendar.prototype.weekDrawTime = function() {
    ...
    $(this.element)
      .find('div.calendar-timeline')
      .css('padding-top', this.conf.weekday.dayline.heightPx + 'px');
  ...
  Calendar.prototype.monthDrawWeek = function() {
    ...
    for (var i = 0; i < this.conf.month.dayheader.weekdays.length; i++) {
      var time = moment()
        .startOf('week')
        .add(this.conf.month.dayheader.weekdays[i], 'days');
      var li = $('<li>', {
    ...
    var maxHeight = 0;
    $(this.element)
      .find('.calendar-month-day-header')
      .each(function() {
        var e = $(this).find('div');
    ...
    while (parseInt(time.format('MM')) == month) {
      ...
      li.append(
        $('<span>')
          ...
          .text(time.format(this.conf.month.weekline.format))
  ...
  Calendar.prototype.monthDrawWeekDays = function() {
    ...
    while (i > 0) {
      ...
      var targetUl = this.element
        .find(".calendar-events>ul>.calendar-month-day-header>div:contains('" + this.miscUcfirstString(time.format(this.conf.month.dayheader.format)) + "')")
        .parent()
        .find('ul');
      var li = $('<li>');
    ...
    for (var i = 0; i < days.length; i++) {
      var targetUl = this.element
        .find(".calendar-events>ul>.calendar-month-day-header>div:contains('" + this.miscUcfirstString(moment.unix(days[i]).format(this.conf.month.dayheader.format)) + "')")
        .parent()
        .find('ul');
      var li = $('<li>', {
  ...
  Calendar.prototype.hoverCategory = function() {
    ...
    this.element.find('.calendar-label').hover(
      function() {
        ...
        setTimeoutConst = setTimeout(function() {
          if (elem.hasClass('calendar-label-daynote')) {
            $(self.element).trigger('Calendar.category-daynote-mouseenter', [self, elem]);
          } else {
            $(self.element).trigger('Calendar.category-event-mouseenter', [self, elem]);
          }
      ...
      function() {
        ...
        elem = $(this);
        if (elem.hasClass('calendar-label-daynote')) {
          $(self.element).trigger('Calendar.category-daynote-mouseleave', [self, elem]);
        } else {
          $(self.element).trigger('Calendar.category-event-mouseleave', [self, elem]);
        }
      }
    );
  };
  ...
  Calendar.prototype.clickCategory = function() {
    ...
    this.element.find('.calendar-label').click(function(event) {
      if ($(event.target).hasClass('calendar-label-daynote')) {
        $(self.element).trigger('Calendar.category-daynote-click', [self, elem]);
      } else {
        $(self.element).trigger('Calendar.category-event-click', [self, elem]);
      }
    });
  };
  ...
  Calendar.prototype.positionEventsInWeek = function() {
    ...
    this.element.find('.calendar-event').each(function(i) {
      ...
      zero =
        parseInt(
          $(this)
            .closest('li.calendar-events-day')
            .attr('data-time')
        ) +
        self.conf.weekday.timeline.fromHour * 60 * 60;
      if (zero > zeroLast) {
        zeroLast = zero;
      }
      ...
      $(this)
        ...
        .each(function(i) {
          var eventTop =
            parseInt(
              $(this)
                .closest('.calendar-month-events-day')
                .index()
            ) *
              self.conf.month.weekline.heightPx +
            20 -
            1;
  ...
  Calendar.prototype.weekDrawEvents = function() {
    for (var i = 0; i < this.events.length; i++) {
      ...
      if (day === false) {
        ...
        this.drawEventsOrDaynotes(day, i, moment.unix(e.start).format('X'), moment.unix(e.end).format('X'), e.title, e.content, e.category, this.getEventCategoryColor(e.category), moment.unix(e.start).format(this.conf.weekday.timeline.format) + ' - ' + moment.unix(e.end).format(this.conf.weekday.timeline.format), ['calendar-event']);
      }
    }
  };
  ...
  Calendar.prototype.monthDrawEvents = function() {
    for (var i = 0; i < this.events.length; i++) {
      ...
      day = false;
      $(this.element)
        .find('.calendar-month-events-day')
        .each(function(i, d) {
    ...
    if (this.conf.now.enable) {
      ...
      var top =
        ((moment().format('X') -
          moment()
            .startOf('day')
            .add(this.conf.weekday.timeline.fromHour, 'h')
            .format('X')) /
          60 /
          this.conf.weekday.timeline.intervalMinutes) *
          this.conf.weekday.timeline.heightPx -
        this.conf.weekday.timeline.heightPx / 2;
      hr.css('top', top + 'px');
      this.element
        .find(
          'li.calendar-events-day[data-time="' +
            moment()
              .startOf('day')
              .format('X') +
            '"]'
        ...
        setInterval(function() {
          ...
          var top =
            ((moment().format('X') -
              moment()
                .startOf('day')
                .add(self.conf.weekday.timeline.fromHour, 'h')
                .format('X')) /
              60 /
              self.conf.weekday.timeline.intervalMinutes) *
              self.conf.weekday.timeline.heightPx -
            self.conf.weekday.timeline.heightPx / 2;
          hr.css('top', top + 'px');
          self.element
            .find(
              'li.calendar-events-day[data-time="' +
    ...
    this.element.find('.calendar-event').each(function() {
      ...
      $(this).hover(
        function() {
          ...
          setTimeoutConst = setTimeout(function() {
            if (elem.hasClass('calendar-daynote')) {
              $(self.element).trigger('Calendar.daynote-mouseenter', [self, elem]);
            } else {
              $(self.element).trigger('Calendar.event-mouseenter', [self, elem]);
            }
        ...
        function() {
          ...
          elem = $(this);
          if (elem.hasClass('calendar-daynote')) {
            $(self.element).trigger('Calendar.daynote-mouseleave', [self, elem]);
          } else {
            $(self.element).trigger('Calendar.event-mouseleave', [self, elem]);
          }
        }
      );
    });
  ...
  Calendar.prototype.clickEventOrDaynote = function() {
    ...
    this.element.find('.calendar-event').each(function() {
      $(this).click(function(event) {
        ...
        if (elem.hasClass('calendar-daynote')) {
          $(self.element).trigger('Calendar.daynote-click', [self, elem, self.daynotes[parseInt(elem.attr('data-index'))]]);
        } else {
          $(self.element).trigger('Calendar.event-click', [self, elem, self.events[parseInt(elem.attr('data-index'))]]);
        }
      });
    });
  };
  ...
  Calendar.prototype.resizeTimeline = function() {
    for (var j = 0; j < this.events.length; j++) {
      ...
      if (
        parseInt(this.events[j].end) >
        parseInt(
          moment
            .unix(this.events[j].end)
            .startOf('day')
            .add(this.conf.weekday.timeline.toHour, 'hour')
            .format('X')
        )
      ) {
        ...
        days.push(
          parseInt(
            moment
              ...
              .startOf('week')
              .add(this.conf.weekday.dayline.weekdays[i], 'days')
              .format('X')
          )
        );
      }
  ...
  Calendar.prototype.moveCurrentIntervalView = function(direction) {
    ...
    if (direction == 'left') {
      this.conf.unixTimestamp = parseInt(
        moment
          .unix(self.conf.unixTimestamp)
          .subtract(1, word)
          .format('X')
      );
    }
  ...
  Calendar.prototype.calculateCurrentInterval = function() {
    var days = this.getViewDays();
    this.fromTimestamp = parseInt(
      moment
        .unix(days[0])
        .startOf('day')
        .format('X')
    );
    this.toTimestamp = parseInt(
      moment
        .unix(days[days.length - 1])
        .endOf('day')
        .format('X')
    );
  };
